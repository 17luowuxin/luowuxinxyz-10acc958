...
export function shouldSendSticker(
  ...
  const strongEmotionalKeywords = [
    ...
    '好饿', '吃饭', '干饭', '饿死', '想吃',
    '好困', '困死了', '困了', '要睡着了',
    '爱你', '喜欢你', '么么哒', '亲亲', '比心', '想你', '抱抱',
    '加油', '冲鸭', '你可以的', '一定行', '相信你',
    '真的吗', '天啊', '我的天', '太震惊了', '不敢相信',
    '呜呜', '好委屈', '心疼', '哭了', '难过', '伤心',
    '太棒了', '绝了', '牛逼', '厉害', '可以的', '没问题',
    '摸鱼', '躺平', '偷懒', '划水'
  ];

  const weakEmotionalKeywords = [
    '开心', '高兴', '好耶', '哈哈', '嘻嘻', '嘿嘿', '哇', '耶',
    '没问题', '可以', '行', 'ok', 'OK', '好的', '好滴', '好哒',
    '困', '累', '懒', '休息', '歇会',
    '震惊', '惊讶', '什么', '居然', '竟然', '卧槽', '我靠',
    '😊', '😢', '😴', '😍', '💕', '❤', '🥺', '😭', '😆', '🥰', '😘',
    '棒', '赞', '好', '行', '嗯嗯', '对', '是', '乖',
    '饿', '吃', '睡', '起床', '工作', '上班'
  ];
  ...
  // 提高触发几率：强情绪75%，弱情绪50%
  let p = 0;
  if (hasStrongEmotion) p = 0.75;
  else if (hasWeakEmotion) p = 0.50;
  else return false;

  // 5) 回复越长越像"认真解释"，降低但不完全禁止
  if (reply.length > 400) p *= 0.85;
  if (reply.length > 800) p *= 0.7;
